...
namespace RatKing.SUS {
	...
	public class Decider : MonoBehaviour {
		public class DebugDraw { public object target; public System.Func<float, DebugDraw, float> draw; public DebugDisplayMode display; public Vector2 scrollPosDebug; }

		public static Decider instance;
		public static Decider Inst => instance != null ? instance : (instance = new GameObject("<DECIDER>").AddComponent<Decider>());
		List<DebugDraw> debugDraws = new List<DebugDraw>();
		public void AddDebugDraw(object target, System.Func<float, DebugDraw, float> debugUI, DebugDisplayMode mode) {
		...
		void OnGUI() {
			...
			for (int i = 0, count = debugDraws.Count; i < count; ++i) {
				var dd = debugDraws[i];
				if (dd.target == null || dd.display == DebugDisplayMode.None || dd.target == null || dd.target.Equals(null)) { debugDraws.RemoveAt(i); --i; --count; continue; }
				drawX += dd.draw.Invoke(drawX, debugDraws[i]);
			}
	...
	public class Decider<T> where T : class {
		...
		public Decider(T target, DebugDisplayMode debugDisplayMode = DebugDisplayMode.None) {
			if (debugDisplayMode != DebugDisplayMode.None) { Decider.Inst.AddDebugDraw(target, DebugGUI, debugDisplayMode); }
			this.target = target;
...
#if ALLOW_IMGUI_DEBUG
		float DebugGUI(float drawX, Decider.DebugDraw dd) {
